...
CSV Files -> Airflow ETL -> raw слой -> staging очищенный слой -> monthly_summary витрина -> Metabase dashboard

### Raw layer

raw — это слой для приема сырых данных из CSV;
...
Повторный запуск DAG физически добавляет новые строки в `raw` с новым `load_id`.
Это ожидаемое поведение, так как, raw слой хранит историю загрузок и позволяет отследить какие данные были загружены в каждом запуске.
Актуальное очищенное состояние данных без дублей формируется на следующем слое — `staging` который строится из последнего успешного батча по сущности и месяцу.
